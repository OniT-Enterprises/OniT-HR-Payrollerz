...
export function normalizeBillingPricing(raw: unknown): BillingPricingConfig {
  ...
  const annualMonthsCharged =
    typeof input.annualMonthsCharged === "number" &&
...
export interface AnnualSeatUpdate {
  quantity: number;
  prorationBehavior: "none" | "always_invoice";
}
 ...
 */
export function effectiveAnnualPaidSeats(
  storedPaidSeats: number | undefined,
  storedTermStart: number | undefined,
  currentTermStart: number | undefined,
  currentQuantity: number,
): number {
  const current = Math.max(1, Math.floor(currentQuantity));
  const sameTerm =
    typeof storedTermStart === "number" &&
  ...
  if (!sameTerm) return current;
  return Math.max(Math.floor(storedPaidSeats as number), current);
}
 ...
 */
export function planAnnualSeatUpdates(
  currentQuantity: number,
  ...
  const updates: AnnualSeatUpdate[] = [];
  if (paid > current)
