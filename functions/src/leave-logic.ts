...
export interface AnnualCarryOverPolicy {
  allowed: boolean;
  /** Maximum days that may roll into the next year (0 disables carry-over). */
  maxDays: number;
}
 ...
 */
export function annualCarryOverPolicyFromConfig(
  configData: Record<string, unknown> | undefined,
): AnnualCarryOverPolicy {
  const policies = configData?.timeOffPolicies as Record<string, unknown> | undefined;
  const annual = policies?.annualLeave as Record<string, unknown> | undefined;
  const allowed = annual?.carryOverAllowed !== false;
  const perType = Number(annual?.maxCarryOverDays);
  const topLevel = Number(policies?.maxCarryOverDays);
 ...
 */
export function computeAnnualCarryOver(options: {
  targetYear: number;
  firstTrackedYear: number | null;
  annualEntitlement: number;
  committedInYear: (year: number) => number;
  policy: AnnualCarryOverPolicy;
}): number {
  const { targetYear, firstTrackedYear, annualEntitlement, committedInYear, policy } = options;
  if (!policy.allowed || policy.maxDays <= 0) return 0;
  if (!Number.isInteger(targetYear)) return 0;
  if (firstTrackedYear === null || !Number.isInteger(firstTrackedYear)) return 0;
  if (firstTrackedYear >= targetYear) return 0;
  if (!Number.isFinite(annualEntitlement) || annualEntitlement < 0) return 0;
 ...
 */
export function firstTrackedAnnualYear(
  requests: Array<{ leaveType?: string; status?: string; startDate?: string }>,
): number | null {
  let first: number | null = null;
  for (const request of requests) {
    if (request.leaveType !== "annual") continue;
    if (request.status !== "pending" && request.status !== "approved") continue;
    if (!request.startDate || !/^\d{4}-\d{2}-\d{2}$/.test(request.startDate)) continue;
 ...
 */
export function probationEligibleFromDate(
  hireDate: string | undefined,
  probationMonths: number,
): string | null {
  if (!Number.isFinite(probationMonths) || probationMonths <= 0) return null;
