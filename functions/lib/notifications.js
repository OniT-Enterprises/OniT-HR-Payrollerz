...
exports.notifyEkipaDocumentRequest = exports.notifyEkipaPayslipReady = exports.notifyEkipaExpenseDecision = exports.notifyEkipaLeaveDecision = exports.notifyEkipaAnnouncement = void 0;
/**
...
function localized(language, copy) {
    return copy[language] || copy.tet;
}
async function getDeviceTargets(userIds) {
    const uniqueUserIds = [...new Set(userIds.filter(Boolean))];
    const snapshots = await Promise.all(uniqueUserIds.map((uid) => db.collection(`users/${uid}/devices`).get()));
    const targets = [];
    for (const snapshot of snapshots) {
        for (const device of snapshot.docs) {
            const data = device.data();
            if (data.enabled === false || data.provider !== "expo")
                continue;
...
async function sendPush(targets, makeCopy) {
    if (targets.length === 0)
        return;
    for (let offset = 0; offset < targets.length; offset += EXPO_BATCH_SIZE) {
        ...
        const messages = chunk.map((target) => {
            ...
            return {
                ...
                title: copy.title,
                body: copy.body,
                data: Object.assign({ route: copy.route, type: copy.type, tenantId: copy.tenantId }, (copy.entityId ? { entityId: copy.entityId } : {})),
            };
        });
...
async function deliverSafely(eventName, targetsPromise, makeCopy) {
    try {
        const targets = await targetsPromise;
        await sendPush(targets, makeCopy);
        v2_1.logger.info("Ekipa push notification delivered", {
            eventName,
        ...
        v2_1.logger.error("Ekipa push notification failed", { eventName, error });
    }
}
exports.notifyEkipaAnnouncement = (0, firestore_2.onDocumentCreated)({
    ...
    await deliverSafely("announcement.created", getTenantTargets(tenantId), () => ({
...
exports.notifyEkipaLeaveDecision = (0, firestore_2.onDocumentUpdated)({
    ...
    await deliverSafely(`leave.${after.status}`, getEmployeeTargets(tenantId, employeeId), (language) => ({
        title: approved
            ? localized(language, {
...
exports.notifyEkipaExpenseDecision = (0, firestore_2.onDocumentUpdated)({
    ...
    await deliverSafely(`expense.${after.status}`, getEmployeeTargets(tenantId, employeeId), (language) => ({
        title: approved
            ? localized(language, {
...
exports.notifyEkipaPayslipReady = (0, firestore_2.onDocumentCreated)({
    ...
    await deliverSafely("payslip.ready", getEmployeeTargets(tenantId, employeeId), (language) => ({
...
exports.notifyEkipaDocumentRequest = (0, firestore_2.onDocumentUpdated)({
    ...
    if (!before || !after || before.status === after.status)
        return;
    if (after.status !== "ready" && after.status !== "rejected")
        return;
    const tenantId = event.params.tenantId;
    const employeeId = event.params.employeeId;
    const ready = after.status === "ready";
    await deliverSafely(`document_request.${after.status}`, getEmployeeTargets(tenantId, employeeId), (language) => ({
        title: ready
            ? localized(language, {
