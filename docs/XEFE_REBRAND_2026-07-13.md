...
1. **[TEST] Google sign-in on xefe.tl** — should read *"to continue to auth.xefe.tl"* and complete. Final confirmation of the auth-domain flip.
...
4. **Cloud Functions `DEFAULT_APP_URL`** — changed to `https://xefe.tl` in source, but **CI does not deploy functions**. Won't be live until a manual `firebase deploy --only functions`.
