...
async function main() {
  ...
  for (const c of subcols) {
    ...
    if (count === 0) continue;
    if (CONFIRM) {
      // recursiveDelete so any nested subcollections go too (e.g. payruns holds
      // a `payslips` subcollection; a shallow batch delete would orphan them).
      await db.recursiveDelete(c);
      console.log(`   🗑  ${c.id}: deleted ${count} (recursive)`);
      grand += count;
    } else {
  ...
  for (const { collection: name, field } of ROOT_ALT_FIELD_SWEEPS) {
    ...
    if (CONFIRM) {
      const n = await deleteQuery(q);
      console.log(`   🗑  ${name} by ${field}: deleted ${n}`);
      grand += n;
    } else {
      console.log(`   •  ${name} by ${field}: ${count} docs`);
      grand += count;
    }
  }
  for (const name of ROOT_COLLECTIONS_RECURSIVE) {
    ...
    if (count === 0) continue;
    if (CONFIRM) {
      const n = await deleteQueryRecursive(q);
      console.log(`   🗑  ${name} (+subcollections): deleted ${n}`);
      grand += n;
    } else {
      console.log(`   •  ${name} (+subcollections): ${count} docs`);
      grand += count;
    }
  }
  for (const name of ROOT_DOC_ID_COLLECTIONS) {
    ...
    if (CONFIRM) {
      await ref.delete();
      console.log(`   🗑  ${name}/${TID}: deleted 1`);
      grand += 1;
    } else {
      console.log(`   •  ${name}/${TID}: 1 doc`);
      grand += 1;
    }
  }
