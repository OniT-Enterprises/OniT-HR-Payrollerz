...
- **Open threads / reviewer feedback**: `docs/FEEDBACK_TRIAGE.md` is the INBOX for anything raised but not resolved — review feedback with its triage verdict, and cross-cutting unfinished work. Add to it rather than leaving an item in a chat log; a **deliberate no** with its reason is a valid outcome. Note the repo is PUBLIC, so raw stakeholder notes do NOT go in GitHub Issues.
...
- **Bank payments**: Read `docs/BANK_PAYMENTS.md` before touching `client/lib/bank-transfers/*`, the Bank Transfers page, or anything that names a government collection account. BNU/BNCTL take salary batches by emailed Excel pack + signed payment order (evidence-based), NOT CSV upload; bank-facing text stays Portuguese. There is **one ATTL account per tax account** — all four in `ATTL_TAX_ACCOUNT_DETAILS` (`client/lib/tlBanking.ts`), evidence-attested, with the instalment account also taking the ANNUAL income tax. Services tax has **no sighted IBAN**: print its local `A/C`, never synthesise one from the TL38/002/…/62 pattern. The credit description is `<TIN>_<SHORT NAME>` and the TIN goes FIRST — ATTL reconciles by it and BNU truncates the field.
