...
- **Payroll money chain**: Read `docs/MONEY_CHAIN.md` (diagrams) before touching payroll run statuses, settlement, payroll/tax journals, leaver final pay, or rules around `payruns`/`taxFilings` — it maps run lifecycle → journals → statutory filings and lists the non-negotiable invariants. §4a carries the **scope contract** for a leaver's once-only Art. 56 severance / Art. 44 subsídio: entitlement and the amount netted off it must ALWAYS be computed over the same scope, from equally-recorded data. Four separate money bugs in Jul 2026 were that one mistake — three of them inside the fix for the previous one — so treat any change on one side as requiring the other.
