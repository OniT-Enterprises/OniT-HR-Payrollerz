  ...
  return (
    <div className="p-3 rounded-lg bg-muted/50 space-y-1 text-sm">
      <div className="flex justify-between">
        ...
        <span>{fmtReminderCurrency(invoice.total)}</span>
      </div>
...
function OverdueWarning({ daysOverdue }: { daysOverdue: number }) {
...
/** Previous reminders info line */
function PreviousReminders({ invoice }: { invoice: Invoice }) {
  ...
  return (
    <AlertDialog open={open} onOpenChange={(o) => !o && onClose()}>
      <AlertDialogContent>
        ...
        <ReminderInvoiceSummary invoice={invoice} isOverdue={isOverdue} />
        {isOverdue && <OverdueWarning daysOverdue={daysOverdue} />}
