  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        <TableCell>
          <div className="flex items-center gap-3">
            <div className="relative flex-shrink-0">
              ...
              {data.isEdited && (
                <div className="absolute -top-0.5 -right-0.5 h-2.5 w-2.5 rounded-full bg-amber-500 border-2 border-background" title="Modified" />
              )}
            </div>
            <div>
        ...
        <TableCell className="text-right">
          <Input
            ...
            aria-label={`Regular hours for ${employeeName}`}
        ...
        <TableCell className="text-right">
          <Input
            ...
            onClick={(e) => e.stopPropagation()}
            onKeyDown={(e) => e.stopPropagation()}
            aria-label={`Overtime hours for ${employeeName}`}
        ...
        <TableCell className="text-right">
          <Input
            ...
            onClick={(e) => e.stopPropagation()}
            onKeyDown={(e) => e.stopPropagation()}
            aria-label={`Night shift hours for ${employeeName}`}
            className={`w-16 text-right ${
        ...
        <TableCell className="text-right">
          <Input
            ...
            onClick={(e) => e.stopPropagation()}
            onKeyDown={(e) => e.stopPropagation()}
            aria-label={`Bonus for ${employeeName}`}
        ...
        <TableCell>
          {data.isEdited && (
            <Button
              ...
              aria-label={`Reset ${employeeName} to original values`}
              onClick={(e) => {
                e.stopPropagation();
                onReset(employeeId);
              }}
      ...
      {isExpanded && data.calculation && (
        <TableRow className="bg-muted/20 hover:bg-muted/20" role="region" aria-label={`Detailed breakdown for ${employeeName}`}>
          <TableCell colSpan={11}>
            <div className="p-4 space-y-3 ml-4 border-l-2 border-green-300 dark:border-green-700">
              {/* Earnings */}
              <div>
                <p className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider mb-2">Earnings</p>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                  ...
                  {data.calculation.subsidioAnual > 0 && (
                    <div className="p-3 rounded-lg bg-background border border-border/50">
                      <p className="text-xs text-muted-foreground">13th Month</p>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <p className="text-[11px] font-semibold text-red-600 dark:text-red-400 uppercase tracking-wider mb-2">Employee Deductions</p>
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div className="p-3 rounded-lg bg-red-50/50 dark:bg-red-950/10 border border-red-500/10">
                      <p className="text-xs text-muted-foreground">INSS (4%)</p>
                      <p className="font-semibold text-sm text-red-600">-{formatCurrencyTL(data.calculation.inssEmployee)}</p>
                    </div>
                  </div>
                </div>
                <div>
                  <p className="text-[11px] font-semibold text-amber-600 dark:text-amber-400 uppercase tracking-wider mb-2">Employer Contributions</p>
                  <div className="grid grid-cols-2 gap-2">
                    <div className="p-3 rounded-lg bg-amber-50/50 dark:bg-amber-950/10 border border-amber-500/10">
                      <p className="text-xs text-muted-foreground">INSS (6%)</p>
              ...
              <div className="flex gap-4 text-xs text-muted-foreground/70 pt-1 border-t border-border/30">
                <span>Taxable income: {formatCurrencyTL(data.calculation.taxableIncome)}</span>
                <span>INSS base: {formatCurrencyTL(data.calculation.inssBase)}</span>
              </div>
            </div>
          </TableCell>
        </TableRow>
