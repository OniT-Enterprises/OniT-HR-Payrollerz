...
import { Building, CheckCircle2, Circle, Globe, Settings } from "lucide-react";
  ...
  return (
    <>
      ...
      >
        <ReportSection
          icon={CheckCircle2}
          title={t("reports.setup.progress.title")}
          description={t("reports.setup.progress.description")}
        >
          <div className="mb-5">
        ...
        <ReportExportCard
          icon={Settings}
          title={t("reports.setup.cards.configuration.title")}
          description={t("reports.setup.cards.configuration.description")}
          rows={[
            {
              icon: Building,
              label: t("reports.setup.cards.configuration.company"),
              value:
            ...
            {
              icon: Globe,
              label: t("reports.setup.cards.configuration.country"),
              value: settings.companyDetails.country || "Timor-Leste",
              tone: "plain",
            },
            {
              icon: CheckCircle2,
              label: t("reports.setup.cards.configuration.status"),
              value: setupProgress.isComplete
                ? t("reports.setup.values.complete")
                : t("reports.setup.values.inProgress"),
              tone: setupProgress.isComplete ? "positive" : "attention",
            },
          ]}
          exportLabel={t("reports.setup.cards.configuration.export")}
          onExport={exportSystemConfig}
        />
      </ReportPage>
    </>
  );
